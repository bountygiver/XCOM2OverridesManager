...
WRITE_OVERRIDE = manager_config.getboolean(CFG_SECTION, "WriteOverride")
...
# "W:\Games\Steam\steamapps\common\XCOM 2\XCom2-WarOfTheChosen\XComGame\Mods"
Path_WOTCMods = manager_config[CFG_SECTION]["WOTCMods"]
# "W:\Games\Steam\steamapps\workshop\content\268500"
Path_SteamMods = manager_config[CFG_SECTION]["SteamMods"]
...
class OverridesManager(object):
    ...
    def __init__(self):
        ...
        chkWOTC = Checkbutton(
            option_frame, text="Process WOTC?", variable=self.IsWOTC)
        chkWOTC.pack(side=LEFT)
        chkClean = Checkbutton(
            option_frame, text="Clean Active Mods (If mods somehow refuse to activate/deactivate)", variable=self.CleanMods)
        chkClean.pack(side=LEFT)
        chkOver = Checkbutton(
            option_frame, text="Write Overrides (Recommended to keep this off)", variable=self.WriteOverride)
        chkOver.pack(side=LEFT)
        chkDry = Checkbutton(
            option_frame, text="Dry Run (Do nothing, see what would be changed in logs)", variable=self.DryRun)
        chkDry.pack(side=LEFT)

        option_frame.pack()

        self.btnLaunch = Button(
            self.window, command=self.start_clean, text="Start")
        self.btnLaunch.pack()
    ...
    def init_paths(self):
        ...
        if is_xcom_path_valid(x_path):
            ...
            # Check is WOTC
            IS_WOTC = self.IsWOTC.get() and is_xcom_path_valid(
                os.path.join(x_path, "XCom2-WarOfTheChosen"))
            manager_config.set(CFG_SECTION, "WOTC",
                               value='True' if IS_WOTC else 'False')
            print("Is WOTC:", IS_WOTC)

            CLEAN_ACTIVE_MODS = self.CleanMods.get() > 0
            manager_config.set(CFG_SECTION, "CleanActiveMods",
                               value='True' if CLEAN_ACTIVE_MODS else 'False')
            print("CleanActiveMods:", CLEAN_ACTIVE_MODS)

            DRY_RUN = self.DryRun.get() > 0
            manager_config.set(CFG_SECTION, "DryRun",
                               value='True' if DRY_RUN else 'False')
            print("DryRun:", DRY_RUN)

            WRITE_OVERRIDE = self.WriteOverride.get() > 0
            manager_config.set(CFG_SECTION, "WriteOverride",
                               value='True' if WRITE_OVERRIDE else 'False')
            print("WriteOverride:", WRITE_OVERRIDE)
            ...
            temp_path = os.path.abspath(os.path.join(
                x_path, "../../workshop/content/268500"))
            if os.path.exists(temp_path):
            ...
            if IS_WOTC:
                temp_path = os.path.abspath(os.path.join(
                    x_path, "XCom2-WarOfTheChosen/XComGame/Mods"))
                if os.path.exists(temp_path):
            ...
            XCE_FILE_PATH = os.path.expanduser(
                '~') + XCOM2_CONF_PATH + XCE_FILE_NAME
            XCMO_FILE_PATH = os.path.expanduser(
                '~') + XCOM2_CONF_PATH + XCMO_FILE_NAME
            ...
            print("Found and Parsed ModClassOverrides: %s" %
                  len(self.found_overrides))
    ...
    def find_inis_in_mods_path(cls, mods_path):
        ...
        print("%s 'XComEngine.ini' files found in mods path: '%s'" %
              (len(engine_ini_paths), mods_path))
    ...
    def _get_existing_overrides(self):
        ...
        self.previous_overrides = self.xce.get_overrides_from_file(
            self.xce.file_path)
        print("Previous Overrides in XComEngine.ini: %s" %
              len(self.previous_overrides))
            ...
            for ini_path in self.find_inis_in_mods_path(mod_path):
                ...
                for found_override in overs:
                    ...
                    if source_mod_name is not None and source_mod_name in self.xcmo.active_mods:
                        ...
                        print("Ignoring override from inactive Mod: %s" %
                              source_mod_name)
    ...
    def _determine_overrides_to_add(self):
        ...
        if self.previous_overrides:
            new_overrides = list(set(self.found_overrides) -
                                 set(self.previous_overrides))
        return new_overrides

    def _determine_overrides_to_remove(self):
        removed_overrides = []
        if self.previous_overrides:
            removed_overrides = list(
                set(self.previous_overrides) - set(self.found_overrides))
    ...
    def _determine_if_changes_needed(self):
        ...
        for new_override in new_overrides:
            print("Will add: %s - Source File: %s" %
                  (new_override, new_override.source_file))

        removed_overrides = self._determine_overrides_to_remove()
        for removed_override in removed_overrides:
            print("Will remove: %s - Source File: %s" %
                  (removed_override, removed_override.source_file))
    ...
    def process_overrides_and_write_config(self):
        if self._determine_if_changes_needed():
            ...
            print(
                "== Updating overrides in 'XComEngine.ini' in user config folder ('%s')" % XCE_FILE_PATH)
            text = self.xce.get_text()
            if WRITE_OVERRIDE and self.found_overrides:
                clean_text = IniTextProcessor.replace_old_overrides(
                    text, self.found_overrides)
            else:
                # No new overrides to add, just clean up instead
                clean_text = IniTextProcessor.clean_out_all_overrides(text)

            print(
                "== Doing cleanup of 'XComEngine.ini' in user config folder ('%s')" % XCE_FILE_PATH)
        ...
        print("== Doing cleanup of 'XComModOptions.ini' in user config folder ('%s')" %
              XCE_FILE_PATH)
        self.xcmo.repair_active_mods(
            dry_run=DRY_RUN, clean_mods=CLEAN_ACTIVE_MODS)
        self.dcmo.repair_default_mods(
            dry_run=DRY_RUN, clean_mods=CLEAN_ACTIVE_MODS)
