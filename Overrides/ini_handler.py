...
class BaseIniHandler(object):
    ...
    def write_text(self, new_text, dry_run=False):
        ...
        if dry_run:
            ...
            with open(self.file_path, "w") as output_file:
                print(
                    "== Writing changes to ini file in user config folder: '%s'" % self.file_path)
    ...
    def backup(self, dry_run=False):
        ...
        if dry_run:
            ...
            print("== Backing up existing '%s' to '%s'" %
                  (self.file_path, bak_path))
...
class XComModOptionsIniHandler(BaseIniHandler):
    ...
    def _parse_active_mods(self):
        ...
        if not lines[0].strip().startswith("[Engine.XComModOptions]"):
            raise ValueError(
                ...
                "Expected %s, got: %s" % (
                    self.file_path, "[Engine.XComModOptions]", lines[0])
            )
        ...
        for line in lines:
            ...
            if len(parts) < 2:
                raise ValueError(
                    "Invalid XComModOptions.ini! Problem on line %s : %s" % (line_counter, line))
    ...
    def repair_active_mods(self, dry_run=False, clean_mods=False):
        re_xmo = re.compile(
            r'\[Engine.XComModOptions\][\s\S^\[]]*\[', flags=re.MULTILINE)
...
class XComEngineIniHandler(BaseIniHandler):
    ...
    def get_overrides_from_file(cls, file_path):
        ...
        for line in ini_lines:
            # Ignore blanks and commented lines
            ...
            if match:
                if len(match.groups()) > 1:
                    raise Exception(
                        "Multiple matches for a single line - Badly formatted ini file: %s" % file_path)
                overrides.append(ModClassOverride.from_raw_line(
                    line.rstrip(), source_file=file_path))
        return overrides
